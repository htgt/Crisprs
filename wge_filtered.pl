...
#
# this is a duplication of wge_test. i'm leaving the other as it is,
# in case we can somehow get the query quicker. for now we are going to
# filter the results first, then run this on valid.bed, insert ids.
# do we even need to store them? we need to do this 1 crispr at a time,
# so might have to split the bed file. 
#
